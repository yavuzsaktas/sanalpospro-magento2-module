...
namespace Sanalpospro\SanalPosPro\Console\Command;
...
class CaptureInitiatedPayments extends Command
{
    ...
    protected function configure(): void
    {
        $this->setName('paythor:capture-initiated')
            ...
            ->addOption(
                ...
                'Show what would be captured without actually calling the API.'
            )
            ->addOption(
                self::OPT_ORDER_ID,
                null,
                InputOption::VALUE_OPTIONAL,
                'Process a single order by increment ID (e.g. 000000030).'
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($collection as $order) {
            ...
            if ($isDryRun) {
                $output->writeln(
                    "  DRY   #{$order->getIncrementId()} | token="
                    . substr($processToken, 0, 16)
                    . "... | {$amount} {$currency}"
                );
                $succeeded++;
                continue;
            ...
            if ($ok) {
                $output->writeln(
                    "<info>  OK    #{$order->getIncrementId()} — capture acknowledged.</info>"
                );
                $succeeded++;
            } else {
                $output->writeln(
                    "<error>  FAIL  #{$order->getIncrementId()} — capture returned error "
                    . "(see paythor_sanalpospro.log).</error>"
                );
                $failed++;
            }
        }
