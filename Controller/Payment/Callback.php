...
namespace Sanalpospro\SanalPosPro\Controller\Payment;
...
use Sanalpospro\SanalPosPro\Model\Order\PaymentStateManager;
...
class Callback implements HttpGetActionInterface, CsrfAwareActionInterface
{
        ...
        try {
            ...
            if ($alreadyCreatedOrderId > 0) {
                ...
                try {
                    ...
                    // Best-effort: ignore session failures so the redirect still succeeds.
                    unset($ignored);
                }
            ...
            $order->addCommentToStatusHistory(
                __(
                    'Paythor: browser-redirect callback received.'
                    . ' Awaiting server webhook for final confirmation. (process status: %1)',
                    $processResult['status']
                )
            );
            $this->orderRepository->save($order);
    ...
    private function handlePostMessageBridge(): ResultInterface
    {
        ...
        $successUrlJson = json_encode($successUrl, $jsonFlags);
        $failureUrlJson = json_encode($failureUrl, $jsonFlags);
